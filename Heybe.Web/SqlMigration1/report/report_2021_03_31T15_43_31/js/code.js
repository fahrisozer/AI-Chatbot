...
const parentDiv = document.getElementById('content-stat-buttons-tab-0');
...
document.addEventListener('mouseup', function (e) {
    parentDiv.removeEventListener('mousemove', handleMouseMove, false);
});
...
function handleMouseMove (e) {
    leftSide.style.width = (e.x - (dividerWidth / 2));
    rightSide.style.width = sqlDiv.clientWidth - leftSide.clientWidth - dividerWidth - 2;
}
...
function updateLeftRightSide () {
    leftSide.style.width = sqlDiv.clientWidth / 2;
    rightSide.style.width = sqlDiv.clientWidth - leftSide.clientWidth - dividerWidth - 2;
}
...
window.addEventListener('load', function () {
    // In order to display correctly initially, content must have width: 100%, or 'full-width'.
    // However width: 100% causes issues when dragging to resize source/target editors.
    // Once window is loaded, we remove 'full-width' so resize will work properly.
    document.getElementById('content-stat-buttons-tab-0').classList.remove('full-width');
    updateLeftRightSide();
});
